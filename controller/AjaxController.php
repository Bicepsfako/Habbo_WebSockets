...
class ajaxController
{
    ...
    public function RegisterUser()
    {
        ...
        $type = false;
        $username = $functions->FilterText($_REQUEST['username']);
        $email = $functions->FilterText($_REQUEST['email']);
        $pass = $functions->HoloHashMD5($_REQUEST['password']);
        ...
        if (!$username || !$email || !$pass || !$_REQUEST['password_2'])
        {
        ...
        else if ($_REQUEST['password'] != $_REQUEST['password_2'])
        {
        ...
        {
            session_start();
            $mysqli->query("INSERT INTO users(`username`, `password`, `mail`, `account_created`, `last_online`, `ip_reg`, `ip_last`, `auth_ticket`) VALUES(
                                    '" . $username . "',
                                    '" . $pass . "',
                                    '" . $email . "',
                                    '" . time() . "',
                                    ...
                                    '" . $_SERVER['REMOTE_ADDR'] . "',
                                    '" . $_SERVER['REMOTE_ADDR'] . "',
                                    '" . $functions->GenerateTicket() . "')") or die($mysqli->error);
