...
namespace Wandala.Communication.ConnectionManager
{
    public class ConnectionHandling
    {
        ...
        private void manager_connectionEvent(ConnectionInformation connection)
        {
            ...
            WandalaEnvironment.GetGame().GetClientManager().CreateAndStartClient(Convert.ToInt32(connection.getConnectionID()), connection);
        }
        ...
        private void CloseConnection(ConnectionInformation Connection)
        {
            ...
            {
                Connection.Dispose();
                WandalaEnvironment.GetGame().GetClientManager().DisposeConnection(Convert.ToInt32( Connection.getConnectionID()));
            }
            catch (Exception e)
            {
