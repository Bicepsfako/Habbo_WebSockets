...
using Wandala.HabboHotel.Items.Data;
...
namespace Wandala.Communication.Packets.Incoming.Rooms.Engine
{
    class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Room.GetRoomItemHandler().GetWallAndFloor.Count() > Convert.ToInt32(WandalaEnvironment.GetSettingsManager().TryGetValue("room.item.placement_limit")))
            {
                Session.SendNotification("You cannot have more than " + Convert.ToInt32(WandalaEnvironment.GetSettingsManager().TryGetValue("room.item.placement_limit")) + " items in a room!");
                return;
            }
            ...
            if (!Item.IsWallItem)
            {
                ...
                if (Room.GetRoomItemHandler().SetFloorItem(Session, RoomItem, X, Y, Rotation, true, false, true))
                {
                    ...
                    if (Session.GetHabbo().Id == Room.OwnerId)
                        WandalaEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_RoomDecoFurniCount", 1, false);
