...
namespace Wandala.Communication.Packets.Incoming.Groups
{
    class JoinGroupEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            List<Group> Groups = WandalaEnvironment.GetGame().GetGroupManager().GetGroupsForUser(Session.GetHabbo().Id);
            ...
            {
                List<GameClient> GroupAdmins = (from Client in WandalaEnvironment.GetGame().GetClientManager().GetClients.ToList() where Client != null && Client.GetHabbo() != null && Group.IsAdmin(Client.GetHabbo().Id) select Client).ToList();
