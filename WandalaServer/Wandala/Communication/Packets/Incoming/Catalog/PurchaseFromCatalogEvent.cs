...
using Wandala.Communication.Packets.Outgoing.Inventory.Pets;
...
using Wandala.HabboHotel.Badges;
...
namespace Wandala.Communication.Packets.Incoming.Catalog
{
    public class PurchaseFromCatalogEvent : IPacketEvent
    {
        ...
        {
            if (WandalaEnvironment.GetSettingsManager().TryGetValue("catalog.enabled") != "1")
            {
                ...
                case InteractionType.LANDSCAPE:
                    ...
                    try
                    {
                        if (string.IsNullOrEmpty(ExtraData))
                            ...
                            Number = Double.Parse(ExtraData, WandalaEnvironment.CultureInfo);
                    }
                    catch (Exception e)
                    {
                        ExceptionLogger.LogException(e);
                        ...
                        case InteractionType.DEAL:
                            {
                                ...
                                if (WandalaEnvironment.GetGame().GetCatalog().TryGetDeal(Item.Data.BehaviourData, out deal))
                                {
                        ...
                        case InteractionType.ROOMDEAL:
                            {
                                ...
                                if (WandalaEnvironment.GetGame().GetCatalog().TryGetDeal(Item.Data.BehaviourData, out deal))
                                {
                                    Room room = WandalaEnvironment.GetGame().GetRoomManager().LoadRoom(deal.RoomId);
                                    if (room == null)
                                    ...
                                    RoomData newRoom = WandalaEnvironment.GetGame().GetRoomManager().CreateRoom(Session, room.Name, room.Description, room.ModelName, room.Category, 10, room.TradeSettings, room.Wallpaper, room.Floor, room.Landscape, room.WallThickness, room.FloorThickness);
                                    if (newRoom == null)
                                        return;

                                    Room myRoom = WandalaEnvironment.GetGame().GetRoomManager().LoadRoom(newRoom.Id);
                                    if (myRoom != null)
                                    {
                ...
                case "p":
                    {
                        ...
                        if (GeneratedPet != null)
                        {
                            ...
                            if (WandalaEnvironment.GetGame().GetItemManager().GetItem(320, out PetFood))
                            {
            ...
            if(!string.IsNullOrEmpty(Item.Badge) &&
                WandalaEnvironment.GetGame().GetBadgeManager().TryGetBadge(Item.Badge, out Badge) &&
