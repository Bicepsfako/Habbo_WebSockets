...
namespace Wandala.Communication.Packets.Outgoing.Groups
{
    class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
            ...
            base.WriteString((WandalaEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "No room found.." : WandalaEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name
            ...
            base.WriteString(WandalaEnvironment.GetUsernameById(Group.CreatorId));
