    ...
    class UsersComposer : ServerPacket
    {
        ...
        private void WriteUser(RoomUser User)
        {
            if (!User.IsPet && !User.IsBot)
            {
                ...
                if (Habbo != null)
                {
                    if (Habbo.GetStats() != null)
                    {
                        ...
                        {
                            if (!WandalaEnvironment.GetGame().GetGroupManager().TryGetGroup(Habbo.GetStats().FavouriteGroupId, out Group))
                                Group = null;
                        }
                    }
                }
            ...
            else if (User.IsBot)
            {
                ...
                base.WriteString(WandalaEnvironment.GetUsernameById(User.BotData.ownerID)); // Owner name
