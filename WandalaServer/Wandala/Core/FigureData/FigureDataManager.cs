...
using log4net;
using Wandala.Core.FigureData.Types;
using System.Xml;
using Wandala.HabboHotel.Catalog.Clothing;
using Wandala.HabboHotel.Users.Clothing.Parts;

namespace Wandala.Core.FigureData
{
    public class FigureDataManager
    {
        private static readonly ILog log = LogManager.GetLogger("Wandala.Core.FigureData");
        ...
        public string ProcessFigure(string figure, string gender, ICollection<ClothingParts> clothingParts, bool hasHabboClub)
        {
            ...
            {
                ICollection<ClothingItem> purchasableParts = WandalaEnvironment.GetGame().GetCatalog().GetClothingManager().GetClothingAllParts;
