...
using Wandala.HabboHotel.Users.Process;
using Wandala.Communication.Packets.Outgoing.Inventory.Purse;


using Wandala.HabboHotel.Users.Navigator.SavedSearches;
...
using Wandala.HabboHotel.Rooms.Chat.Commands;
using Wandala.HabboHotel.Users.Permissions;
using Wandala.HabboHotel.Subscriptions;
using Wandala.HabboHotel.Users.Calendar;
using Wandala.HabboHotel.Users.Ignores;

namespace Wandala.HabboHotel.Users
{
    public class Habbo
    {
        private static readonly ILog log = LogManager.GetLogger("Wandala.HabboHotel.Users");
            ...
            int GOTWPoints, bool IgnoreInvites, double TimeMuted, double TradingLock, bool AllowGifts, int FriendBarState, bool DisableForcedEffects, bool AllowMimic, int VIPRank)
        {
            ...
            this._footballLook = WandalaEnvironment.FilterFigure(Look.ToLower());
            ...
            if (this._tradingLockExpiry > 0 && WandalaEnvironment.GetUnixTimestamp() > this.TradingLockExpiry)
            {
            ...
            this._sessionStart = WandalaEnvironment.GetUnixTimestamp();
            this._messengerSpamCount = 0;
            this._messengerSpamTime = 0;
            this._creditsTickUpdate = Convert.ToInt32(WandalaEnvironment.GetSettingsManager().TryGetValue("user.currency_scheduler.tick"));
                ...
                try
                {
                    ...
                    if (Convert.ToString(StatRow["respectsTimestamp"]) != DateTime.Today.ToString("MM/dd"))
                    {
                        ...
                        if (this._permissions.HasRight("mod_tool"))
                            DailyRespects = 20;
                        else if (WandalaEnvironment.GetGame().GetSubscriptionManager().TryGetSubscriptionData(VIPRank, out SubData))
            ...
            Group G = null;
            if (!WandalaEnvironment.GetGame().GetGroupManager().TryGetGroup(this._habboStats.FavouriteGroupId, out G))
        ...
        public Room CurrentRoom
        {
            get
            {
                ...
                Room _room = null;
                if (WandalaEnvironment.GetGame().GetRoomManager().TryGetRoom(CurrentRoomId, out _room))
                    return _room;
        ...
        public void OnDisconnect()
        {
            ...
            WandalaEnvironment.GetGame().GetClientManager().UnregisterClient(Id, Username);
            ...
            if (!this._habboSaved)
            {
                ...
                using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.RunQuery("UPDATE `users` SET `online` = '0', `last_online` = '" + WandalaEnvironment.GetUnixTimestamp() + "', `activity_points` = '" + this.Duckets + "', `credits` = '" + this.Credits + "', `vip_points` = '" + this.Diamonds + "', `home_room` = '" + this.HomeRoom + "', `gotw_points` = '" + this.GOTWPoints + "', `time_muted` = '" + this.TimeMuted + "',`friend_bar_state` = '" + FriendBarStateUtility.GetInt(this._friendbarState) + "' WHERE id = '" + Id + "' LIMIT 1;UPDATE `user_stats` SET `roomvisits` = '" + this._habboStats.RoomVisits + "', `onlineTime` = '" + (WandalaEnvironment.GetUnixTimestamp() - this.SessionStart + this._habboStats.OnlineTime) + "', `respect` = '" + this._habboStats.Respect + "', `respectGiven` = '" + this._habboStats.RespectGiven + "', `giftsGiven` = '" + this._habboStats.GiftsGiven + "', `giftsReceived` = '" + this._habboStats.GiftsReceived + "', `dailyRespectPoints` = '" + this._habboStats.DailyRespectPoints + "', `dailyPetRespectPoints` = '" + this._habboStats.DailyPetRespectPoints + "', `AchievementScore` = '" + this._habboStats.AchievementPoints + "', `quest_id` = '" + this._habboStats.QuestID + "', `quest_progress` = '" + this._habboStats.QuestProgress + "', `groupid` = '" + this._habboStats.FavouriteGroupId + "',`forum_posts` = '" + this._habboStats.ForumPosts +"' WHERE `id` = '" + this.Id + "' LIMIT 1;");
        ...
        public void CheckCreditsTimer()
        {
            try
            {
                ...
                {
                    int CreditUpdate = Convert.ToInt32(WandalaEnvironment.GetSettingsManager().TryGetValue("user.currency_scheduler.credit_reward"));
                    int DucketUpdate = Convert.ToInt32(WandalaEnvironment.GetSettingsManager().TryGetValue("user.currency_scheduler.ducket_reward"));

                    SubscriptionData SubData = null;
                    if (WandalaEnvironment.GetGame().GetSubscriptionManager().TryGetSubscriptionData(this._vipRank, out SubData))
                    {
                    ...
                    this.CreditsUpdateTick = Convert.ToInt32(WandalaEnvironment.GetSettingsManager().TryGetValue("user.currency_scheduler.tick"));
                }
            }
            catch { }
        }
        ...
        public GameClient GetClient()
        {
            ...
            return WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(Id);
        }
        ...
        {
            this.LastNameChange = WandalaEnvironment.GetUnixTimestamp();
        ...
        public void PrepareRoom(int Id, string Password)
        {
            ...
            {
                Room OldRoom = null;
                if (!WandalaEnvironment.GetGame().GetRoomManager().TryGetRoom(this.GetClient().GetHabbo().CurrentRoomId, out OldRoom))
                    return;
            ...
            Room Room = WandalaEnvironment.GetGame().GetRoomManager().LoadRoom(Id);
            if (Room == null)
            {
                this.GetClient().SendPacket(new CloseConnectionComposer());
                return;
        ...
        public bool EnterRoom(Room Room)
        {
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.RunQuery("INSERT INTO user_roomvisits (user_id,room_id,entry_timestamp,exit_timestamp,hour,minute) VALUES ('" + this.GetClient().GetHabbo().Id + "','" + this.GetClient().GetHabbo().CurrentRoomId + "','" + WandalaEnvironment.GetUnixTimestamp() + "','0','" + DateTime.Now.Hour + "','" + DateTime.Now.Minute + "');");// +
            }
            ...
            if (Room.OwnerId != this.Id)
            {
                ...
                WandalaEnvironment.GetGame().GetAchievementManager().ProgressAchievement(this.GetClient(), "ACH_RoomEntry", 1);
            }
            return true;
        }
    }
