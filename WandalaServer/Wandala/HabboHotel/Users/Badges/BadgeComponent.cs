...
namespace Wandala.HabboHotel.Users.Badges
{
    public class BadgeComponent
    {
        ...
        public BadgeComponent(Habbo Player, UserData.UserData data)
        {
            ...
            foreach (Badge badge in data.badges)
            {
                ...
                if (!WandalaEnvironment.GetGame().GetBadgeManager().TryGetBadge(badge.Code, out BadgeDefinition) || BadgeDefinition.RequiredRight.Length > 0 && !Player.GetPermissions().HasRight(BadgeDefinition.RequiredRight))
                    continue;
        ...
        public void GiveBadge(string Badge, Boolean InDatabase, GameClient Session)
        {
            ...
            BadgeDefinition BadgeDefinition = null;
            if (!WandalaEnvironment.GetGame().GetBadgeManager().TryGetBadge(Badge.ToUpper(), out BadgeDefinition) || BadgeDefinition.RequiredRight.Length > 0 && !Session.GetHabbo().GetPermissions().HasRight(BadgeDefinition.RequiredRight))
                return;
