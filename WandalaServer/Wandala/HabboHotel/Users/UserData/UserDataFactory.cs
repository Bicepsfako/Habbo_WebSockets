    ...
    public class UserDataFactory
    {
        public static UserData GetUserData(string SessionTicket, out byte errorCode)
        {
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(UserId) != null)
                {
                    errorCode = 2;
                    WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(UserId).Disconnect();
                    return null;
                }
            ...
            foreach (DataRow dRow in dFriends.Rows)
            {
                ...
                bool friendHideOnline = WandalaEnvironment.EnumToBool(dRow["hide_online"].ToString());
                bool friendHideRoom = WandalaEnvironment.EnumToBool(dRow["hide_inroom"].ToString());
            ...
            {
                rooms.Add(WandalaEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(dRow["id"]), dRow));
            }
        ...
        public static UserData GetUserData(int UserId)
        {
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                WandalaEnvironment.GetGame().GetClientManager().LogClonesOut(Convert.ToInt32(UserId));
