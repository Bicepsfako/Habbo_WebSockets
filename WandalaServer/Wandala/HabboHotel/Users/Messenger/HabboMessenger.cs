...
namespace Wandala.HabboHotel.Users.Messenger
{
    public class HabboMessenger
    {
        ...
        public void ProcessOfflineMessages()
        {
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetMessages != null)
                {
                    GameClient Client = WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
                    if (Client == null)
                        return;
        ...
        public void Destroy()
        {
            IEnumerable<GameClient> onlineUsers = WandalaEnvironment.GetGame().GetClientManager().GetClientsById(_friends.Keys);
        ...
        public void CreateFriendship(int friendID)
        {
            ...
            GameClient User = WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (User != null)
                WandalaEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User, "ACH_FriendListSize", 1);

            GameClient thisUser = WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(_userId);
            if (thisUser != null)
                WandalaEnvironment.GetGame().GetAchievementManager().ProgressAchievement(thisUser, "ACH_FriendListSize", 1);
        }
        ...
        {
            GameClient friend = WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (friend == null || friend.GetHabbo() == null)
            {
                ...
                newFriend = new MessengerBuddy(friendID, Convert.ToString(dRow["username"]), Convert.ToString(dRow["look"]), Convert.ToString(dRow["motto"]), Convert.ToInt32(dRow["last_online"]),
                    WandalaEnvironment.EnumToBool(dRow["hide_online"].ToString()), WandalaEnvironment.EnumToBool(dRow["hide_inroom"].ToString()));
            }
            else
            {
        ...
        public bool RequestBuddy(string UserQuery)
        {
            ...
            GameClient client = WandalaEnvironment.GetGame().GetClientManager().GetClientByUsername(UserQuery);
            if (client == null)
            {
                DataRow Row = null;
                using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
                ...
                hasFQDisabled = WandalaEnvironment.EnumToBool(Row["block_newfriends"].ToString());
            }
            else
            {
            ...
            WandalaEnvironment.GetGame().GetQuestManager().ProgressUserQuest(GetClient(), QuestType.ADD_FRIENDS);

            GameClient ToUser = WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (ToUser == null || ToUser.GetHabbo() == null)
                return true;

            MessengerRequest Request = new MessengerRequest(ToId, _userId, WandalaEnvironment.GetGame().GetClientManager().GetNameById(_userId));

            ToUser.GetHabbo().GetMessenger().OnNewRequest(_userId);

            UserCache ThisUser = WandalaEnvironment.GetGame().GetCacheManager().GenerateUser(_userId);
        ...
        public void OnNewRequest(int friendID)
        {
            if (!_requests.ContainsKey(friendID))
                _requests.Add(friendID, new MessengerRequest(_userId, friendID, WandalaEnvironment.GetGame().GetClientManager().GetNameById(friendID)));
        }
        ...
        public void SendInstantMessage(int ToId, string Message)
        {
            ...
            {
                GetClient().GetHabbo().MessengerSpamTime = WandalaEnvironment.GetUnixTimestamp() + 60;
            ...
            else if (GetClient().GetHabbo().MessengerSpamTime > WandalaEnvironment.GetUnixTimestamp())
            {
                double Time = GetClient().GetHabbo().MessengerSpamTime - WandalaEnvironment.GetUnixTimestamp();
            ...
            GameClient Client = WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (Client == null || Client.GetHabbo() == null || Client.GetHabbo().GetMessenger() == null)
            {
                using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
                {
        ...
        {
            IEnumerable<GameClient> MyFriends = WandalaEnvironment.GetGame().GetClientManager().GetClientsById(this._friends.Keys);
        ...
        {
            return WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
        }
