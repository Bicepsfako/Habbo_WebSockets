...
namespace Wandala.HabboHotel.Cache.Process
{
    sealed class ProcessComponent
    {
        private static readonly ILog log = LogManager.GetLogger("Wandala.HabboHotel.Cache.Process.ProcessComponent");
        ...
        public void Run(object State)
        {
            try
            {
                ...
                // BEGIN CODE
                List<UserCache> CacheList = WandalaEnvironment.GetGame().GetCacheManager().GetUserCache().ToList();
                if (CacheList.Count > 0)
                {
                    foreach (UserCache Cache in CacheList)
                    {
                        try
                        {
                            ...
                            if (Cache.isExpired())
                                WandalaEnvironment.GetGame().GetCacheManager().TryRemoveUser(Cache.Id, out Temp);
                ...
                List<Habbo> CachedUsers = WandalaEnvironment.GetUsersCached().ToList();
                    ...
                    foreach (Habbo Data in CachedUsers)
                    {
                        try
                        {
                            ...
                            if (Data.CacheExpired())
                                WandalaEnvironment.RemoveFromCache(Data.Id, out Temp);
