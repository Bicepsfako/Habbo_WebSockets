...
namespace Wandala.HabboHotel.Moderation
{
    public sealed class ModerationManager
    {
        private static ILog log = LogManager.GetLogger("Wandala.HabboHotel.Moderation.ModerationManager");
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetBans != null)
                {
                    foreach (DataRow dRow in GetBans.Rows)
                    {
                        ...
                        if (Ban != null)
                        {
                            if (expires > WandalaEnvironment.GetUnixTimestamp())
                            {
        ...
        public void BanUser(string Mod, ModerationBanType Type, string BanValue, string Reason, double ExpireTimestamp)
        {
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("REPLACE INTO `bans` (`bantype`, `value`, `reason`, `expire`, `added_by`,`added_date`) VALUES ('" + BanType + "', '" + BanValue + "', @reason, " + ExpireTimestamp + ", '" + Mod + "', '" + WandalaEnvironment.GetUnixTimestamp() + "');");
        ...
        public bool MachineBanCheck(string MachineId)
        {
            ...
            if (WandalaEnvironment.GetGame().GetModerationManager().IsBanned(MachineId, out MachineBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        WandalaEnvironment.GetGame().GetModerationManager().RemoveBan(MachineId);
                        return false;
                    }
                }
            }
        ...
        public bool UsernameBanCheck(string Username)
        {
            ...
            if (WandalaEnvironment.GetGame().GetModerationManager().IsBanned(Username, out UsernameBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        WandalaEnvironment.GetGame().GetModerationManager().RemoveBan(Username);
                        return false;
                    }
                }
            }
