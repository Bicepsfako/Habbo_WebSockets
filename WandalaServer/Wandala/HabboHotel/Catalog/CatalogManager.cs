...
using Wandala.HabboHotel.Catalog.Pets;
using Wandala.HabboHotel.Catalog.Vouchers;
using Wandala.HabboHotel.Catalog.Marketplace;
using Wandala.HabboHotel.Catalog.Clothing;

using Wandala.Database.Interfaces;

namespace Wandala.HabboHotel.Catalog
{
    public class CatalogManager
    {
        private static readonly ILog log = LogManager.GetLogger("Wandala.HabboHotel.Catalog.CatalogManager");
                ...
                if (CatalogueItems != null)
                {
                    foreach (DataRow Row in CatalogueItems.Rows)
                    {
                        ...
                        this._items[PageId].Add(Convert.ToInt32(Row["id"]), new CatalogItem(Convert.ToInt32(Row["id"]), Convert.ToInt32(Row["item_id"]),
                            ...
                            Convert.ToInt32(Row["amount"]), Convert.ToInt32(Row["limited_sells"]), Convert.ToInt32(Row["limited_stack"]), WandalaEnvironment.EnumToBool(Row["offer_active"].ToString()),
