        ...
        public static void GenerateExtradata(Item Item, ServerPacket Message)
        {
            switch (Item.GetBaseItem().InteractionType)
            {
                ...
                case InteractionType.GUILD_FORUM:
                    Group Group = null;
                    if (!WandalaEnvironment.GetGame().GetGroupManager().TryGetGroup(Item.GroupId, out Group))
                    {
                        Message.WriteInteger(1);
                        Message.WriteInteger(0);
                        Message.WriteString(Item.ExtraData);
                    ...
                    else
                    {
                        ...
                        Message.WriteString(WandalaEnvironment.GetGame().GetGroupManager().GetColourCode(Group.Colour1, true));
                        Message.WriteString(WandalaEnvironment.GetGame().GetGroupManager().GetColourCode(Group.Colour2, false));
                    }
                    break;
                ...
                case InteractionType.GIFT:
                    {
                        ...
                        else
                        {
                            ...
                            UserCache Purchaser = WandalaEnvironment.GetGame().GetCacheManager().GenerateUser(Convert.ToInt32(ExtraData[2]));
                            if (Purchaser == null)
                            {
                                Message.WriteInteger(0);
                                Message.WriteInteger(0);
                ...
                case InteractionType.TELEVISION:
                    ...
                    //Message.WriteString("http://img.youtube.com/vi/" + WandalaEnvironment.GetGame().GetTelevisionManager().TelevisionList.OrderBy(x => Guid.NewGuid()).FirstOrDefault().YouTubeId + "/3.jpg");
                    Message.WriteString("");
                    break;
