...
namespace Wandala.HabboHotel.Items
{
    public class ItemDataManager
    {
        private static readonly ILog log = LogManager.GetLogger("Wandala.HabboHotel.Items.ItemDataManager");
                ...
                if (ItemData != null)
                {
                    foreach (DataRow Row in ItemData.Rows)
                    {
                        try
                        {
                            ...
                            bool allowStack = WandalaEnvironment.EnumToBool(Row["can_stack"].ToString());
                            bool allowWalk = WandalaEnvironment.EnumToBool(Row["is_walkable"].ToString());
                            bool allowSit = WandalaEnvironment.EnumToBool(Row["can_sit"].ToString());
                            bool allowRecycle = WandalaEnvironment.EnumToBool(Row["allow_recycle"].ToString());
                            bool allowTrade = WandalaEnvironment.EnumToBool(Row["allow_trade"].ToString());
                            bool allowMarketplace = Convert.ToInt32(Row["allow_marketplace_sell"]) == 1;
                            bool allowGift = Convert.ToInt32(Row["allow_gift"]) == 1;
                            bool allowInventoryStack = WandalaEnvironment.EnumToBool(Row["allow_inventory_stack"].ToString());
                            ...
                            bool IsRare = WandalaEnvironment.EnumToBool(Row["is_rare"].ToString());
                            bool ExtraRot = WandalaEnvironment.EnumToBool(Row["extra_rot"].ToString());
