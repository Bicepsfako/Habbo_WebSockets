...
using Wandala.HabboHotel.Permissions;
...
using Wandala.Communication;

namespace Wandala.HabboHotel.GameClients
{
    public class GameClient
    {
        ...
        private void parser_onNewPacket(ClientPacket Message)
        {
            try
            {
                WandalaEnvironment.GetGame().GetPacketManager().TryExecutePacket(this, Message);
            }
            catch (Exception e)
            {
        ...
        {
            _connection.SendData(WandalaEnvironment.GetDefaultEncoding().GetBytes("<?xml version=\"1.0\"?>\r\n" +
        ...
        public bool TryAuthenticate(string AuthTicket)
        {
            try
            {
                ...
                {
                    if (WandalaEnvironment.GetGame().GetModerationManager().IsBanned(MachineId, out BanRecord))
                    {
                        if (WandalaEnvironment.GetGame().GetModerationManager().MachineBanCheck(MachineId))
                        {
                            Disconnect();
                            return false;
                        }
                ...
                if (userData.user != null)
                {
                    ...
                    BanRecord = null;
                    if (WandalaEnvironment.GetGame().GetModerationManager().IsBanned(userData.user.Username, out BanRecord))
                    {
                        if (WandalaEnvironment.GetGame().GetModerationManager().UsernameBanCheck(userData.user.Username))
                        {
                            Disconnect();
                            return false;
                        }
                ...
                WandalaEnvironment.GetGame().GetClientManager().RegisterClient(this, userData.userID, userData.user.Username);
                ...
                if (_habbo != null)
                {
                    ...
                    SendPacket(new CfhTopicsInitComposer(WandalaEnvironment.GetGame().GetModerationManager().UserActionPresets));

                    SendPacket(new BadgeDefinitionsComposer(WandalaEnvironment.GetGame().GetAchievementManager()._achievements));
                    ...
                    if (WandalaEnvironment.GetGame().GetPermissionManager().TryGetGroup(_habbo.Rank, out PermissionGroup))
                    {
                    ...
                    if (WandalaEnvironment.GetGame().GetSubscriptionManager().TryGetSubscriptionData(this._habbo.VIPRank, out SubData))
                    {
                    ...
                    if (!WandalaEnvironment.GetGame().GetCacheManager().ContainsUser(_habbo.Id))
                        WandalaEnvironment.GetGame().GetCacheManager().GenerateUser(_habbo.Id);

                    _habbo.Look = WandalaEnvironment.GetFigureManager().ProcessFigure(this._habbo.Look, this._habbo.Gender, this._habbo.GetClothing().GetClothingParts, true);
                    _habbo.InitProcess();
                    ...
                    if (WandalaEnvironment.GetSettingsManager().TryGetValue("user.login.message.enabled") == "1")
                        SendPacket(new MOTDNotificationComposer(WandalaEnvironment.GetLanguageManager().TryGetValue("user.login.message")));

                    WandalaEnvironment.GetGame().GetRewardManager().CheckRewards(this);
                    return true;
                }
            }
            catch (Exception e)
