...
namespace Wandala.HabboHotel.Navigator
{
        ...
        public static void Search(ServerPacket Message, SearchResultList SearchResult, string SearchData, GameClient Session, int FetchLimit)
        {
            ...
            switch (SearchResult.CategoryType)
            {
                ...
                case NavigatorCategoryType.QUERY:
                    {
                        ...
                        if (SearchData.ToLower().StartsWith("owner:"))
                        {
                            if (SearchData.Length > 0)
                            {
                                ...
                                if (GetRooms != null)
                                {
                                    ...
                                    {
                                        RoomData RoomData = WandalaEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                        ...
                        else if (SearchData.ToLower().StartsWith("tag:"))
                        {
                            ...
                            ICollection<RoomData> TagMatches = WandalaEnvironment.GetGame().GetRoomManager().SearchTaggedRooms(SearchData);
                        ...
                        else if (SearchData.ToLower().StartsWith("group:"))
                        {
                            ...
                            ICollection<RoomData> GroupRooms = WandalaEnvironment.GetGame().GetRoomManager().SearchGroupRooms(SearchData);
                            ...
                            if (SearchData.Length > 0)
                            {
                                ...
                                if (Table != null)
                                {
                                    foreach (DataRow Row in Table.Rows)
                                    {
                                        ...
                                        RoomData RData = WandalaEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row);
                ...
                case NavigatorCategoryType.FEATURED:
                    ...
                    ICollection<FeaturedRoom> Featured = WandalaEnvironment.GetGame().GetNavigator().GetFeaturedRooms();
                    foreach (FeaturedRoom FeaturedItem in Featured.ToList())
                    {
                        if (FeaturedItem == null)
                            continue;

                        RoomData Data = WandalaEnvironment.GetGame().GetRoomManager().GenerateRoomData(FeaturedItem.RoomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.POPULAR:
                    {
                        List<RoomData> PopularRooms = WandalaEnvironment.GetGame().GetRoomManager().GetPopularRooms(-1, FetchLimit);
                ...
                case NavigatorCategoryType.RECOMMENDED:
                    {
                        List<RoomData> RecommendedRooms = WandalaEnvironment.GetGame().GetRoomManager().GetRecommendedRooms(FetchLimit);
                ...
                case NavigatorCategoryType.CATEGORY:
                    {
                        List<RoomData> GetRoomsByCategory = WandalaEnvironment.GetGame().GetRoomManager().GetRoomsByCategory(SearchResult.Id, FetchLimit);
                ...
                case NavigatorCategoryType.MY_FAVORITES:
                    ...
                    {
                        RoomData Room = WandalaEnvironment.GetGame().GetRoomManager().GenerateRoomData(Id);
                        if (Room == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_GROUPS:
                    ...
                    foreach (Group Group in WandalaEnvironment.GetGame().GetGroupManager().GetGroupsForUser(Session.GetHabbo().Id).ToList())
                    {
                        if (Group == null)
                            continue;

                        RoomData Data = WandalaEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId);
                        if (Data == null)
                            continue;
                ...
                case NavigatorCategoryType.MY_RIGHTS:
                    ...
                    using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        {
                            RoomData Data = WandalaEnvironment.GetGame().GetRoomManager().GenerateRoomData(Convert.ToInt32(Row["room_id"]));
                            if (Data == null)
                                continue;
                ...
                case NavigatorCategoryType.TOP_PROMOTIONS:
                    {
                        List<RoomData> GetPopularPromotions = WandalaEnvironment.GetGame().GetRoomManager().GetOnGoingRoomPromotions(16, FetchLimit);
                ...
                case NavigatorCategoryType.PROMOTION_CATEGORY:
                    {
                        List<RoomData> GetPromotedRooms = WandalaEnvironment.GetGame().GetRoomManager().GetPromotedRooms(SearchResult.Id, FetchLimit);
