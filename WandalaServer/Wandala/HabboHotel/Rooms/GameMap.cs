...
namespace Wandala.HabboHotel.Rooms
{
    public class Gamemap
    {
        ...
        public Gamemap(Room room)
        {
            ...
            mStaticModel = WandalaEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            if (mStaticModel == null)
            {
                WandalaEnvironment.GetGame().GetRoomManager().LoadModel(room.ModelName);
                mStaticModel = WandalaEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            }
        ...
        public Point getRandomWalkableSquare()
        {
            ...
            int RandomNumber = WandalaEnvironment.GetRandomNumber(0, walkableSquares.Count);
            int i = 0;
        ...
        public bool IsValidStep2(RoomUser User, Vector2D From, Vector2D To, bool EndOfPath, bool Override)
        {
            ...
            if (Items.Count > 0)
            {
                ...
                if (HasGroupGate)
                {
                    ...
                    {
                        Group Group = null;
                        if (!WandalaEnvironment.GetGame().GetGroupManager().TryGetGroup(I.GroupId, out Group))
                            return false;
