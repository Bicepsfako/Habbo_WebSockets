...
namespace Wandala.HabboHotel.Rooms.Chat.Filter
{
        ...
        public void Init()
        {
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (Data != null)
                {
                    ...
                    {
                        this._filteredWords.Add(new WordFilter(Convert.ToString(Row["word"]), Convert.ToString(Row["replacement"]), WandalaEnvironment.EnumToBool(Row["strict"].ToString()), WandalaEnvironment.EnumToBool(Row["bannable"].ToString())));
                    }
                }
            }
        }
