    ...
    public class RoomUser
    {
        ...
        public string GetUsername()
        {
            ...
            if (GetClient() != null)
            {
                if (GetClient().GetHabbo() != null)
                {
                    ...
                    return WandalaEnvironment.GetUsernameById(HabboId);
        ...
        public bool IncrementAndCheckFlood(out int MuteTime)
        {
            ...
            else if (ChatSpamCount >= 6)
            {
                ...
                GetClient().GetHabbo().FloodTime = WandalaEnvironment.GetUnixTimestamp() + MuteTime;
        ...
        public void OnChat(int Colour, string Message, bool Shout)
        {
            ...
            if (Shout)
                Packet = new ShoutComposer(VirtualId, Message, WandalaEnvironment.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
            else
                Packet = new ChatComposer(VirtualId, Message, WandalaEnvironment.GetGame().GetChatManager().GetEmotions().GetEmotionsForText(Message), Colour);
        ...
        public GameClient GetClient()
        {
            ...
            if (mClient == null)
                mClient = WandalaEnvironment.GetGame().GetClientManager().GetClientByUserID(HabboId);
            return mClient;
        }
        ...
        {
            if (mRoom == null)
                if (WandalaEnvironment.GetGame().GetRoomManager().TryGetRoom(RoomId, out mRoom))
                    return mRoom;
