...
using Wandala.Utilities.Enclosure;
using System.Linq;
...
namespace Wandala.HabboHotel.Rooms.Games.Banzai
{
    public class BattleBanzai
    {
        ...
        public void BanzaiStart()
        {
            ...
            timestarted = WandalaEnvironment.GetUnixTimestamp();
        ...
        public void BanzaiEnd(bool userTriggered = false)
        {
            ...
            if (winners != TEAM.NONE)
            {
                ...
                foreach (RoomUser User in Winners.ToList())
                {
                    ...
                    {
                        if (WandalaEnvironment.GetUnixTimestamp() - timestarted > 5)
                        {
                            WandalaEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallTilesLocked", User.LockedTilesCount);
                            WandalaEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallPlayer", 1);
                        }
                    }
                    if (winners == TEAM.BLUE)
                    {
                        ...
                        {
                            if (WandalaEnvironment.GetUnixTimestamp() - timestarted > 5)
                                WandalaEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallWinner", 1);
