...
namespace Wandala.HabboHotel.Rooms
{
    public class RoomData
    {
        ...
        public void Fill(DataRow Row)
        {
            ...
            Group G = null;
            if (WandalaEnvironment.GetGame().GetGroupManager().TryGetGroup(Convert.ToInt32(Row["group_id"]), out G))
                Group = G;
            else
                Group = null;
            ...
            mModel = WandalaEnvironment.GetGame().GetRoomManager().GetModel(ModelName);

            this.PushEnabled = WandalaEnvironment.EnumToBool(Row["push_enabled"].ToString());
            this.PullEnabled = WandalaEnvironment.EnumToBool(Row["pull_enabled"].ToString());
            this.SPushEnabled = WandalaEnvironment.EnumToBool(Row["spush_enabled"].ToString());
            this.SPullEnabled = WandalaEnvironment.EnumToBool(Row["spull_enabled"].ToString());
            this.EnablesEnabled = WandalaEnvironment.EnumToBool(Row["enables_enabled"].ToString());
            this.RespectNotificationsEnabled = WandalaEnvironment.EnumToBool(Row["respect_notifications_enabled"].ToString());
            this.PetMorphsAllowed = WandalaEnvironment.EnumToBool(Row["pet_morphs_allowed"].ToString());
        }
