...
using Wandala.HabboHotel.Rooms.PathFinding;

namespace Wandala.HabboHotel.Rooms.AI.Types
{
    public class PetBot : BotAI
    {
        ...
        public override void OnSelfEnterRoom()
        {
            ...
            //int randomX = WandalaEnvironment.GetRandomNumber(0, GetRoom().Model.MapSizeX);
            //int randomY = WandalaEnvironment.GetRandomNumber(0, GetRoom().Model.MapSizeY);
            if (GetRoomUser() != null)
        ...
        public override void OnUserEnterRoom(RoomUser User)
        {
            if (User.GetClient() != null && User.GetClient().GetHabbo() != null)
            {
                ...
                if (Pet != null)
                {
                    ...
                    {
                        string[] Speech = WandalaEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("welcome.speech.pet" + Pet.PetData.Type);
        ...
        public override void OnTimerTick()
        {
            ...
            if (SpeechTimer <= 0)
            {
                ...
                if (Pet != null)
                {
                    ...
                    string[] Speech = WandalaEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("speech.pet" + Pet.PetData.Type);
                    string rSpeech = Speech[RandomNumber.GenerateRandom(0, Speech.Length - 1)];
                ...
                SpeechTimer = WandalaEnvironment.GetRandomNumber(20, 120);
            }
            else
            {
                SpeechTimer--;
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            if ((Message.ToLower().StartsWith(Pet.PetData.Name.ToLower() + " ") && User.GetClient().GetHabbo().Username.ToLower() == Pet.PetData.OwnerName.ToLower()) || (Message.ToLower().StartsWith(Pet.PetData.Name.ToLower() + " ") && WandalaEnvironment.GetGame().GetChatManager().GetPetCommands().TryInvoke(Message.Substring(Pet.PetData.Name.ToLower().Length + 1)) == 8))
            {
                string Command = Message.Substring(Pet.PetData.Name.ToLower().Length + 1);

                int r = RandomNumber.GenerateRandom(1, 8); // Made Random
                if (Pet.PetData.Energy > 10 && r < 6 || Pet.PetData.Level > 15 || WandalaEnvironment.GetGame().GetChatManager().GetPetCommands().TryInvoke(Command) == 8)
                {
                    RemovePetStatus(); // Remove Status

                    switch (WandalaEnvironment.GetGame().GetChatManager().GetPetCommands().TryInvoke(Command))
                    {
                        ...
                        default:
                            string[] Speech = WandalaEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("pet.unknowncommand");
                ...
                else
                {
                    ...
                    {
                        string[] Speech = WandalaEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("pet.tired");
                    ...
                    {
                        string[] Speech = WandalaEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("pet.lazy");
