...
namespace Wandala.HabboHotel.Rooms.Trading
{
        ...
        public void ProcessItems()
        {
            ...
            using (IQueryAdapter dbClient = WandalaEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                foreach (Item Item in UserOne)
                {
                    ...
                    if (Item.Data.InteractionType == InteractionType.EXCHANGE && WandalaEnvironment.GetSettingsManager().TryGetValue("trading.auto_exchange_redeemables") == "1")
                    {
